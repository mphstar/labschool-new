...
import { ArrowLeft, ArrowUpDown, Award, Eye } from 'lucide-react';
...
export default function Page() {
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        { title: 'Siswa Berprestasi', href: `/kelas/${kelas.id}/prestasi` },
    ];
    ...
    const columns: ColumnDef<DataItem>[] = [
        ...
        {
            id: 'jumlah_prestasi',
            header: 'Jumlah Prestasi',
            cell: ({ row }) => (
                <Badge variant="secondary" className="bg-yellow-100 text-yellow-800">
                    <Award size={12} className="mr-1" />
                    {row.original.prestasi.length} Prestasi
                </Badge>
            ),
        },
        {
            id: 'prestasi_terakhir',
            header: 'Prestasi Terakhir',
            cell: ({ row }) => {
                const lastPrestasi = row.original.prestasi[0];
                return lastPrestasi ? (
                    <span className="max-w-xs truncate block">{lastPrestasi.judul}</span>
                ) : (
        ...
        },
        {
            ...
            cell: ({ row }) => {
                ...
                return (
                    <Link href={`/siswa/${item.siswa.id}/prestasi`}>
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Siswa Berprestasi - ${kelas.name}`} />
            <div className="flex h-full w-full flex-col gap-4 rounded-xl p-4">
                ...
                <DataTable columns={columns} data={data} emptyMessage="Belum ada siswa yang memiliki data prestasi di kelas ini" />
            </div>
        </AppLayout>
    );
}
